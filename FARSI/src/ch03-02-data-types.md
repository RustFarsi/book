...
هر مقداری در راست از یک *نوع داده* خاص است، که به راست می گوید چه نوع داده ای مشخص شده است تا بفهمد که با آن داده چطور رفتار کند. ما به برسی دو نوع از زیر مجموعه های انواع داده ها خواهیم پرداخت: scalar (اسکالر) و compound (مرکب).

به خاطر داشته باشید که راست یک زبان برنامه نویسی استاتیک است، که یعنی باید نوع همه  متغیر ها رو در زمان کامپایل بداند. کاپایلر معمول بر اساس مقدار و نوع استفاده می تواند متوجه نوع داده بشود. در مواقعی که انواع مختلف از داده ها امکان پذیر است، مثل وقتی که  ما یک `String` (رشته) را به یک عدد با استفاده از `parse` تبدیل میکنیم مثل کاری که در قسمت 2 [“برنامه یک بازی حدس‌زدنی“][comparing-the-guess-to-the-secret-number]<!-- ignore --> انجام دادیم، ما باید نوع آن رو مشخص کنیم، مثل این:

```rust
let guess: u32 = "42".parse().expect("Not a number!");
```

اگر اینجا نوع داده رو مشخص نکنیم، راست ارور زیر رو به ما نشان میدهد که یعنی کاپایلر اطلاعت بیشتری از ما می خواهد تا متونجه نوع داده که می خواهیم استفاده کنیم بشود:

```text
{{#include ../listings/ch03-common-programming-concepts/output-only-01-no-type-annotations/output.txt}}
```

در ادامه انواع مختلف انجام این کار را خواهید دید.

## نوع Scalar (اسکالر)

یک داده *scalar* یک مقدار را نشان میدهد، راست 4 نوع داده اصلی scalar دارد:

 integers, floating-point numbers, Booleans,  characters

 شاید شما این ها رو از زبان های برنامه نویسی دیگر بشناسید.
بریم ببینیم این ها چطور در راست کار می کنند.

### انواع Integer ها

یک *integer* یک عدد است بدونه جزء کسری. ما از یک نوع integer در قسمت 2 استفاده کردیم، نوع داده `u32`. این نوع داده یعنی مقدار باید یک عدد غیر منفی (unsigned integer) باشد  و میتواند تا 32 بیت از فضا رو استفاده کند (کل اعداد صحیح رو با `i` و اعداد نامفی رو با `u` مشخص میکنم). جدول 1-3 انواع integer ها در راست رو نشان می دهد. هر کدام از اون  ها می تونه یک نوع داده integer رو مشخص کنه (مثلا `i16`)

<span class="caption">جدول 1-3: انواع داده integer در راست</span>
